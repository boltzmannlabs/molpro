...
class VoxelNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.is_tensor(idx):
            idx = idx.tolist()
        pdbid = self.ids[idx]
        score = self.data_handle[pdbid]['score'][:]
        rot = 0
        tr = (0,0,0)
        rec_grid_1, rec_grid_2 = self.prepare_complex(pdbid,rotation = rot,translation = tr)

        return np.concatenate((rec_grid_1, rec_grid_2)), score
    
    def prepare_complex(self,pdb_id,rotation=0, translation=(0, 0, 0),vmin=0, vmax=1):
        
        prot_coords_1 = self.data_handle[pdb_id]['prot_1_coords'][:]
        prot_coords_2 = self.data_handle[pdb_id]['prot_2_coords'][:]
        prot_features_2 = self.data_handle[pdb_id]['prot_2_features'][:]
        prot_features_1 = self.data_handle[pdb_id]['prot_1_features'][:]
        prot_coords = rotate(prot_coords_1,rotation)
        prot_coords += translation
        poc_coords = rotate(prot_coords_2,rotation)
        poc_coords += translation
        rec_grid_1 = make_grid(prot_coords_1,prot_features_1 ,
                                            max_dist=self.max_dist)
        rec_grid_2 = make_grid(prot_coords_2,prot_features_2,
                                      max_dist=self.max_dist)
        rec_grid_1 = rec_grid_1.transpose(3,0,1,2)
        rec_grid_2 = rec_grid_2.transpose(3,0,1,2)
        return rec_grid, rec_grid_2
        
class VoxelNetDataModule(LightningDataModule):
    ...
    def setup(self, **kwargs):
        ...
        self.train_dataset = VoxelNetDataset(self.hdf_path, self.max_dist, self.grid_resolution,
                                           self.train_ids_path, self.transform)
        self.val_dataset = VoxelNetDataset(self.hdf_path, self.max_dist, self.grid_resolution,
                                           self.valid_ids_path)
